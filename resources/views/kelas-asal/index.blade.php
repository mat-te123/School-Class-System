...
<head>
    ...
    <title>Kelas Asal - School Class System</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="bg-gray-50 font-sans leading-normal tracking-normal" x-data="kelasTable()">

    <!-- Navigation Header -->
    <nav class="bg-emerald-600 shadow-lg text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center space-x-3">
                    <span class="font-bold text-xl tracking-wider">🏫 Kelas Asal</span>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-sm bg-emerald-700 px-3 py-1 rounded-full">Administrator</span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Daftar Kelas Asal (Kelas X)</h1>
                <p class="text-sm text-gray-500 mt-1">
                    Total <span class="font-semibold text-emerald-600" x-text="meta.total || 0"></span> kelas terdaftar.
                </p>
            </div>
            <button @click="openAddModal()"
                    class="inline-flex items-center justify-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-lg transition transform hover:-translate-y-0.5">
                ➕ Tambah Kelas
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col md:flex-row md:items-center gap-4">
            <div class="flex-1">
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">🔍 Cari Kelas</label>
                <input type="text"
                       x-model="search"
                       @input.debounce.500ms="page = 1; fetchKelas()"
                       placeholder="Cari nama kelas..."
                       class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 transition">
            </div>
            <div class="md:w-48">
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Tampilkan</label>
                <select x-model="perPage" @change="page = 1; fetchKelas()"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm bg-white focus:border-emerald-500 focus:ring focus:ring-emerald-200 transition">
        ...
        <div class="bg-white shadow rounded-xl overflow-hidden">
            <div x-show="loading" class="px-6 py-4 text-sm text-emerald-600 font-semibold">⏳ Memuat data...</div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left font-semibold text-gray-600 uppercase tracking-wider">Nama Kelas</th>
                            <th class="px-6 py-3 text-left font-semibold text-gray-600 uppercase tracking-wider">Tingkat</th>
                            <th class="px-6 py-3 text-center font-semibold text-gray-600 uppercase tracking-wider">Kapasitas</th>
                            <th class="px-6 py-3 text-center font-semibold text-gray-600 uppercase tracking-wider">Total Siswa</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <template x-for="(item, index) in kelas" :key="item.id">
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 whitespace-nowrap text-gray-500" x-text="((meta.current_page || 1) - 1) * (meta.per_page || 10) + index + 1"></td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" x-text="item.nama_kelas"></td>
                                <td class="px-6 py-4 whitespace-nowrap text-gray-500" x-text="item.tingkat"></td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-gray-500" x-text="item.kapasitas"></td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800" x-text="item.siswas_count ?? 0"></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                        ...
                        <tr x-show="!loading && kelas.length === 0">
                            <td colspan="7" class="px-6 py-10 text-center text-gray-500">
                                Belum ada kelas yang cocok dengan pencarian.
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="flex items-center space-x-1">
                    ...
                    <template x-for="p in pages" :key="p">
                        <button @click="goToPage(p)"
                                ...
                                :class="p === (meta.current_page || 1) ? 'bg-emerald-600 text-white shadow' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'"
                                x-text="p"></button>
                    </template>
                    <button @click="goToPage((meta.current_page || 1) + 1)"
    ...
    <div x-show="showFormModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto" @click.away="closeFormModal()">
            <div class="flex justify-between items-center border-b pb-3">
                <h3 class="text-xl font-bold text-gray-900" x-text="editId ? '✏️ Edit Kelas' : '➕ Tambah Kelas'"></h3>
            ...
            <form @submit.prevent="submitForm" class="space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.nama_kelas" required maxlength="50" placeholder="X A"
                           class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Kapasitas</label>
                        <input type="number" x-model="form.kapasitas" min="1"
                               class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200">
                    </div>
                    <div x-show="editId">
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Status Aktif</label>
                        <select x-model="form.is_active"
                                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white focus:border-emerald-500 focus:ring focus:ring-emerald-200">
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end space-x-3">
                    ...
                    <button type="submit" :disabled="isSubmitting"
                            class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-2 rounded-lg text-sm transition disabled:opacity-50 disabled:cursor-not-allowed">
                        <span x-show="!isSubmitting" x-text="editId ? 'Simpan Perubahan' : 'Tambah Kelas'"></span>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl">
            <div class="text-center space-y-2">
                ...
                <h3 class="text-lg font-bold text-gray-900">Hapus Kelas?</h3>
                <p class="text-sm text-gray-500">
                    Anda yakin ingin menghapus <span class="font-semibold text-gray-700" x-text="deleteTarget?.nama_kelas"></span>?
            ...
            return {
                ...
                async fetchKelas() {
                    ...
                    try {
                        ...
                        const res = await fetch('/kelas-asal?' + params.toString(), {
                            headers: {
                ...
                async submitForm() {
                    ...
                    try {
                        const url = this.editId ? '/kelas-asal/' + this.editId : '/kelas-asal';
                        ...
                        if (res.ok) {
                            this.formMessage = data.message || 'Data kelas berhasil disimpan.';
                            this.formError = false;
                            this.closeFormModal();
                            await this.fetchKelas();
                        } else {
                ...
                async confirmDelete() {
                    ...
                    this.formMessage = '';
                    try {
                        const res = await fetch('/kelas-asal/' + this.deleteTarget.id, {
                            method: 'DELETE',
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        ...
                        const data = await res.json();
                        if (res.ok) {
                            this.formMessage = data.message || 'Data kelas berhasil dihapus.';
                            this.formError = false;
